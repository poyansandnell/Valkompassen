...
**How to apply:** vid regenerering (t.ex. efter valet 2026): pipeline = webSearch → webFetch (startsida + program-undersidor) → queryWithLLM med strikt prompt och responseSchema. Filen har `questionFingerprint` (första/sista frågetext + antal) som seed.ts validerar — ändras kommunfrågorna måste researchen göras om. Aug 2026: kommunfrågorna omformulerades kommunneutralt (inga ortsnamn, mildare formuleringar); partisvaren behölls eftersom skiljelinjerna är oförändrade, och fingerprintens "first" uppdaterades i takt. OBS: `URL` finns inte i CodeExecution-sandboxens durable scope — normalisera URL:er med regex. Filtrera bort tromanpublik/nyhetssajter/svt-valkompass som "hemsida".
